...
const DOMAIN = process.env.DOMAIN || '项目所提供的域名.com';       // 填写项目域名,不要带前缀"https://"
...
    // **只保留 VLESS，并加入 alpn=http/1.1**
    const vlessURL = `vless://${UUID}@${DOMAIN}:443?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&alpn=http%2F1.1&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${NAME}-${ISP}`;
    
...
    // 如果不是 VLESS，则关闭连接 (原代码中的 Trojan 处理已被移除)
    ws.close();
    
  }).on('error', () => {});
});
